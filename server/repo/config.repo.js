...
const DEFAULTS = { diasUteis: 26, taxaWisePct: 1, taxaConversao: 5, taxaConversaoEur: 5.8, taxaConversaoGbp: 6.5, taxaConversaoAuto: 0 };
...
async function update(competencia, patch, updatedBy) {
  ...
  await db.run(
    `UPDATE config_mes SET dias_uteis = ?, taxa_wise_pct = ?, taxa_conversao = ?, taxa_conversao_eur = ?, taxa_conversao_gbp = ?,
    ...
    [
      ...
      patch.taxaConversaoEur != null ? Number(patch.taxaConversaoEur) : current.taxaConversaoEur,
...
async function syncAutoRates(usdRate, eurRate, gbpRate) {
  ...
  if (eurRate > 0) { sets.push('taxa_conversao_eur = ?'); args.push(eurRate); }
